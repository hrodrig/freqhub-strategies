...
class IchiV1Strategy(IStrategy):
    ...
    def bot_start(self, **kwargs) -> None:
        """
        Runs when the bot starts. Sends a full startup message to Telegram/Mattermost.
        """
        try:
            # Get bot information
            ...
            # Get stake_amount - can be numeric or "unlimited"
            stake_amount_val = None
            
            # Method 1: from self.dp.stake_amount
            if hasattr(self.dp, 'stake_amount') and self.dp.stake_amount is not None:
                stake_amount_val = self.dp.stake_amount
            
            # Method 2: from kwargs['config']
            if stake_amount_val is None and 'config' in kwargs:
                config = kwargs['config']
                if hasattr(config, 'get'):
                    stake_amount_val = config.get('stake_amount')
                elif hasattr(config, 'stake_amount'):
                    stake_amount_val = config.stake_amount
            
            # Method 3: from self.dp.config
            if stake_amount_val is None and hasattr(self.dp, 'config'):
            ...
            # Read full config from file (to get all values)
            config_data = None
            config_paths = [
            ...
            # Method 4: read directly from the config file (fallback)
            if stake_amount_val is None:
                try:
                    for config_path in config_paths:
                        if os.path.exists(config_path):
                            with open(config_path, 'r') as f:
                                config_data = json.load(f)
                                stake_amount_val = config_data.get('stake_amount')
                                if stake_amount_val is not None:
                                    break
                except Exception as e:
                    logger.debug(f"Could not read config file: {e}")
            
            # If we don't have config_data, try reading now for max_open_trades and dry_run
            if config_data is None:
                try:
                    ...
                    logger.debug(f"Could not read config file for other values: {e}")
            
            # Format stake_amount
            if stake_amount_val is None:
                stake_amount = "unlimited"
            elif isinstance(stake_amount_val, str):
                if stake_amount_val.lower() == "unlimited" or stake_amount_val == "":
                    stake_amount = "unlimited"
                else:
                    try:
                        stake_amount = f"{float(stake_amount_val)}"
                    except ValueError:
                        stake_amount = stake_amount_val
            elif isinstance(stake_amount_val, (int, float)):
                stake_amount = f"{stake_amount_val}"
            else:
                stake_amount = str(stake_amount_val)
            
            # Get max_open_trades - try from multiple sources
            max_open_trades_val = None
            if hasattr(self.dp, 'config') and hasattr(self.dp.config, 'get'):
                max_open_trades_val = self.dp.config.get('max_open_trades')
            elif config_data:
                max_open_trades_val = config_data.get('max_open_trades')
            
            max_open_trades = max_open_trades_val if max_open_trades_val is not None else 'N/A'
            
            # Get dry_run status - try from multiple sources
            dry_run_val = True  # Default to True (Dry-Run)
            if hasattr(self.dp, 'config') and hasattr(self.dp.config, 'get'):
                dry_run_val = self.dp.config.get('dry_run', True)
            elif config_data:
                dry_run_val = config_data.get('dry_run', True)
            
            dry_run_status = "Dry-Run" if dry_run_val else "Real"
            
            # Build startup message
            startup_msg = f"""🤖 *FreqTrade Bot Startup - IchiV1*
